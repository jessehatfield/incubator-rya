...
public class MongoPipelineStrategy extends AbstractRuleExecutionStrategy {
    ...
    private final RyaQueryEngine<MongoDBRdfConfiguration> engine;
    private boolean usedBackup = false;

    /**
     * Initialize based on a configuration.
     * @param conf Should contain database information; cannot be null.
     */
    public MongoPipelineStrategy(MongoDBRdfConfiguration conf) throws ForwardChainException {
        Preconditions.checkNotNull(conf);
        final String mongoDBName = conf.getMongoDBName();
        final String collectionName = conf.getTriplesCollectionName();
        ...
        try {
            this.dao = (MongoDBRyaDAO) RyaSailFactory.getRyaDAO(conf);
        } catch (RyaDAOException e) {
            throw new ForwardChainException("Can't connect to Rya.", e);
        }
        this.engine = dao.getQueryEngine();
        this.engine.setConf(conf);
        this.backup = new SailExecutionStrategy(conf, dao);
        final MongoDbBatchWriterConfig writerConfig = MongoDbBatchWriterUtils.getMongoDbBatchWriterConfig(conf);
        ...
        baseCollection.aggregate(pipeline)
            ...
            .forEach(new Block<Document>() {
                ...
                public void apply(Document doc) {
                    ...
                    RyaStatement rstmt = storageStrategy.deserializeDBObject(obj);
